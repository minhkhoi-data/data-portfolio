...
A fixed seed value of **599** is used for reproducibility. The command `set.seed(599)` is used wherever a random procedure is required. In this analysis, the seed affects the 10-fold cross-validation in Question 1, the 80/20 train-test split in Question 2, and the 10-fold cross-validation in Question 2.
...
The scale-location plot shows a reasonably even spread of residuals across fitted values, although some variation remains. Overall, the diagnostic plots suggest that Model B is acceptable for this analysis, but the regression assumptions are not perfectly satisfied.
...
learners <- read.csv("../data/learners.csv", stringsAsFactors = FALSE)
...
This analysis can be reproduced by placing `energy_homes.csv` and `learners.csv` in the `data` folder, opening the same R Markdown file, installing the required packages, and running the report from top to bottom. The fixed seed value used in this report is `D = 599` for reproducibility.
...
If the analysis is rerun with a different seed value, deterministic outputs would stay the same, including the missing-value counts, completion rate, and regression models fitted on the full `energy` dataset. Outputs depending on random folds or the train-test split could change slightly, including the cross-validated MSE values, the selected training and test rows, the logistic regression estimates fitted on the training set, and the test-set performance metrics.
