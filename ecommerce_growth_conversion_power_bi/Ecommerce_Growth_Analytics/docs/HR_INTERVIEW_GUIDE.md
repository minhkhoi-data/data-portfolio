...
> I built an e-commerce growth and conversion dashboard in Power BI using a star-schema model with separate session, order, and order-item fact tables. I used Power Query for data typing and preparation, then created DAX measures for traffic, funnel conversion, revenue, AOV, revenue per session, repeat purchase, and time comparisons. The main business finding was that the largest funnel loss happened between product view and add-to-cart, while mobile generated the most traffic but converted materially below desktop. I also compared acquisition channels using both volume and efficiency rather than traffic alone.
...
> Không. Bản GitHub em bundle là deterministic demo data theo GA4-style schema để project reproducible và dễ mở bằng Power BI. Em có kèm SQL template để cùng semantic model có thể được tạo từ official Google Merchandise Store public BigQuery sample. Em tách rõ demo findings và official-source reproduction path trong README.
