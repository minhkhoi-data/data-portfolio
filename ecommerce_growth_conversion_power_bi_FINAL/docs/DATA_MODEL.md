...
All report calculations are explicit measures stored in `_Measures`. The measures are organized into six display folders: Traffic & Engagement, Funnel, Commercial, Customer, Product, and Time Intelligence.

Technical keys and raw fact columns are hidden from the report field pane. Analysts should filter with shared dimensions and use explicit measures instead of implicit aggregation.
